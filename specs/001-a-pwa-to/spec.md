...
- **FR-014**: The system MUST rely solely on default user agent styling (no custom CSS classes or custom margins/padding) but MAY use minimal structural HTML (line breaks, paragraphs, lists, fieldset/legend, section/article) to group content for readability.
...
- **FR-017**: The system SHOULD prevent duplicate persisted records under rapid double submissions (defined as two submissions within 500ms) and MUST surface exactly one created entity while ignoring/discarding the second action (idempotent client UX). UI remains responsive (no frozen inputs) during suppression.
...
- **FR-036**: The system SHOULD handle recipe instructions of at least 20KB without truncation, validation failure, or client error (no hard limit enforced beyond practical memory constraints) and MUST store text fully.

### Non-Functional Requirements
*(Added for traceability—drawn from implementation plan but absent from original spec)*
- **NFR-001 Performance Startup**: First Contentful Paint < 1.5s and interaction readiness < 2.0s on a mid‑range device (cold load, network throttled to "Fast 3G").
- **NFR-002 Bundle Size**: Initial JS payload (app + vendor) ≤ 150KB gzip.
- **NFR-003 Offline First**: Application shell and last-synced data load successfully while offline with no console errors.
- **NFR-004 Accessibility Baseline**: Semantic landmarks present (`main`, appropriate headings); no custom ARIA unless necessary; no automated a11y violations at WCAG AA critical subset (landmarks, label presence) using a lightweight checker.
- **NFR-005 Styling Constraint**: No custom CSS rules beyond user agent defaults (no external stylesheets except generated by tooling). Minimal structural HTML only.
- **NFR-006 Type Safety**: Zero `any` (unjustified), zero non-null assertions in production source; all external data validated via Zod at boundary.
- **NFR-007 Error/Loading States**: Each route-level async surface has Suspense fallback and Error Boundary with semantic markup.

### Clarifying Notes (De-duplication / Offline Cluster)
FR-009 (offline browse), FR-019 (durable persistence across sessions), and FR-035 (recipe offline availability) together define the offline availability cluster. They are retained separately for traceability but MUST be implemented via a single cohesive offline caching + persistence strategy (Firestore persistence + prefetch loaders). No additional divergence implied.
